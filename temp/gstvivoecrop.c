...
gst_vivoe_crop_transform_frame_ip (GstVideoFilter * filter, GstVideoFrame * frame)
{
	...
	GstElement *videocrop = gst_element_factory_make_log ( "videocrop", "videocrop_roi" );

	gst_element_link_pads (GST_ELEMENT ( vivoecrop ), "sink", videocrop , "sink");
	gst_element_link_pads (GST_ELEMENT (videocrop ), "src", GST_ELEMENT (vivoecrop) , "src");
