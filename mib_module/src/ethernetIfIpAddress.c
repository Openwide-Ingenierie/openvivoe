...
/* define for the name of the list of data that will hold old values of ethernetIfIpAdress, needed to perform undo*/
#define DELAYED_INSTANCE_SET_NAME "list_ethernetIfIpAddress"
                          ...
                          netsnmp_request_info         *requests)
{
    ...
    switch(reqinfo->mode) {
        ...
        case MODE_SET_RESERVE2:
            ...
            xe can reset it in case of undo*/
        ...
        */
        netsnmp_request_add_list_data(requests,
                                      ...
                                      (DELAYED_INSTANCE_SET_NAME,
                                       ethernetIfIpAddress, free));

        case MODE_SET_FREE:
            /* XXX: free resources allocated in RESERVE1 and/or
               RESERVE2.  Something failed somewhere, and the states
               below won't be called. */
               free(&ret); /*free resources allocated in RESERVE1*/
        ...
        case MODE_SET_ACTION:
            ...
            for(i =0; i< strlen(ethernetIfIpAddress); i++){
                ethernetIfIpAddress[i] = requests->requestvb->val.string[i];
            }
            /*get possible errors
            ret = netsnmp_check(ethernetIfIpAddress, ASN_IPADDRESS);
            printf("%d", ret);
            if (ret != SNMP_ERR_NOERROR) {
                netsnmp_set_request_error(reqinfo, requests, ethernetIfIpAddress);
            }
            break;*/
        ...
        case MODE_SET_UNDO:
           ...
           // ethernetIfIpAddress = (u_char*) netsnmp_request_get_list_data(requests, DELAYED_INSTANCE_SET_NAME);
            memcpy(ethernetIfIpAddress, (u_char*) netsnmp_request_get_list_data(requests, DELAYED_INSTANCE_SET_NAME), 4);
            if (ethernetIfIpAddress != SNMP_ERR_NOERROR) {
                /* try _really_really_ hard to never get to this point */
